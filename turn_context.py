...
4. 用中文回复用户"""
    ...
    # Plan Mode V1：复杂任务规划规则（软约束——模型自觉遵守）
    base += """

复杂任务规划规则：
1. 遇到多步骤、多文件或需要设计决策的任务，先用 plan 工具把任务拆成阶段计划
2. 每个阶段必须包含：明确目标 + 可执行的验收条件（能判断该阶段是否真正完成的标准）
3. 每完成一个阶段，必须对照该阶段的验收条件验证结果；不满足则修正后重新验证
4. 全部阶段完成后，做整体验证：逐条核对所有验收条件，全部满足才向用户报告完成"""
