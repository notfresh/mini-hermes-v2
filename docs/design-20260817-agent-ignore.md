...
- **权限码**：`R`/`W`/`X` 任意组合，**= 要排除（禁止）的权限**，像 `.gitignore` 一样只写想禁的（对齐文件命名 ignore）
- 示例：

| 规则 | 含义 |
|------|------|
| `/home/user/.ssh::` | 完全禁止（不可读、写、执行） |
| `/home/user/private::RWX` | 三项全禁（全排除）= 上行等价简写 |
| `/tmp/sandbox::R` | 禁止读，允许 W、X |
| `/data/backup::X` | 禁止执行，允许 R、W |

## 3. 匹配与合并规则（做减法）

- **前缀匹配**：规则路径是目标路径的前缀即命中（目录规则覆盖子树）；文件规则精确匹配
- **合并 = 权限差集**：目标路径最终允许权限 = `RWX` 全集中**减去**所有命中规则声明的权限；无命中 = 默认 `RWX` 全允许
- 因此**多条规则叠加，权限越禁越多、不可逆**——天然实现"父目录禁读，子目录加规则也救不回"，且不需要"最先匹配优先"的排序逻辑（差集天然累积）
...
tools_runner.agent_ignore = AgentIgnore.load_default()   # 当前 CWD 下的 .agentignore，不存在 → None
```
...
4. **AgentIgnore 文件本身可被规则保护**：如 `./.agentignore::` 禁止工具改动它（自保护）
5. **已知局限（如实记录，后续版本处理）**：
